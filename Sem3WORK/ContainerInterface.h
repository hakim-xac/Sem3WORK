...
namespace WORK {
    ...
    public:
        ...
        bool isQuickSearch(const TypeContainer& elem);
        template <typename Iter>
        bool quickSearch(Iter begin, Iter end, const TypeContainer& elem);
    };
}
...
::showQSort(const TypeContainer& elem)
{
    ...
    std::string find{ isQuickSearch(elem) ? "Найдено" : "НЕ найдено" };
    addToStatusBar(delimiter('-'));
